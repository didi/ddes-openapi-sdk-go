...
// UnmarshalJSON 容错反序列化：账单明细字段众多，真实流量中金额（float32）等字段
// 类型不稳定（number/数字str 混存），标准反序列化会失败。用 core.SmartDecode 经
// 中间 map 转换（number↔string 容错），不经 json.Unmarshal 到自身以避免递归。
func (b *BillListItemOfWangYC) UnmarshalJSON(data []byte) error {
	return core.SmartDecode(data, b)
}
